 ...
 *
 * ```ts
 * import { registerDbFactory } from 'vonosan/server'
...
export const dbProvider = createMiddleware<{
  ...
  if (!connectionString) {
    throw new Error(
      '[vonosan] dbProvider: No database connection string found. ' +
  ...
  if (!_dbFactory) {
    throw new Error(
      '[vonosan] dbProvider: No DB factory registered. ' +
  ...
  try {
    ...
    if (client?.end) {
      await client.end().catch((err: unknown) => {
        Logger.error('[vonosan] dbProvider: Error closing DB connection', {
          error: String(err),
        })
      })
    }
