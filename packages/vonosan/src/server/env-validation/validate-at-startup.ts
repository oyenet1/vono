 ...
 * Error format:
 * ```
 * [vonosan] Missing/invalid env vars:
...
export function validateEnvAtStartup<T>(
  ...
  if (!result.success) {
    ...
    const message = `[vonosan] Missing/invalid env vars:\n${errors}`
    ...
    if (refineResult !== true) {
      const message =
        typeof refineResult === 'string'
          ? `[vonosan] Env validation failed:\n  ❌ ${refineResult}`
          : `[vonosan] Env validation failed: refine() returned false`
